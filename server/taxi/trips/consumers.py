import asyncio

from channels.db import database_sync_to_async
from channels.generic.websocket import AsyncJsonWebsocketConsumer
from .serializers import ReadOnlyTripSerializer, TripSerializer
from .models import Trip

class TaxiConsumer(AsyncJsonWebsocketConsumer):

    def __init__(self, scope):
        super().__init__(scope)

        # Keep track of the user's trips
        self.trips = set()


    async def connect(self):
        user = self.scope['user'] # The scope is like Django's request
        if user.is_anonymous:
            await self.close()
        else:
            
            channel_groups = [] # A list of channel operations (add) of each trip

            # Add a driver to the 'drivers' group
            user_group = await self._get_user_group(user)
            if user_group == 'driver':
                channel_groups.append(self.channel_layer.group_add(
                    group='drivers',
                    channel=self.channel_name
                ))

            # Get trips and add rider to each one's group
            self.trips = set([
                # Since trip_id is UUIDField type, you have to convert
                # it to a str type object
                str(trip_id) for trip_id in await self._get_trips(user)
            ])
            for trip in self.trips:
                channel_groups.append(self.channel_layer.group_add(trip, self.channel_name))
            asyncio.gather(*channel_groups) # Let Channel do the scheduling work
            await self.accept()
    

    async def receive_json(self, content, **kwargs):
        message_type = content.get('type')
        if message_type == 'create.trip':
            await self.create_trip(content)
        elif message_type == 'update.trip':
            await self.update_trip(content)


    async def echo_message(self, event):
        await self.send_json(event)


    async def create_trip(self, event):
        trip = await self._create_trip(event.get('data'))
        trip_id = str(trip.id)
        trip_data = ReadOnlyTripSerializer(trip).data

        # Send rider requests to all drivers.
        # TODO: Only broadcast it to drivers nearby
        await self.channel_layer.group_send(group='drivers', message={
            'type': 'echo.message',
            'data': trip_data
        })

        # If the trip is not yet tracked, add it to
        # the trips set and also add a group to
        # the channel layer
        # We don't want to add the rider to the 
        # same trip twice
        if trip_id not in self.trips:
            self.trips.add(trip_id)
            await self.channel_layer.group_add(
                group = trip_id,
                channel = self.channel_name
            )

        await self.send_json({
            'type': 'MESSAGE',
            'data': trip_data
        })


    async def update_trip(self, event):
        trip = await self._update_trip(event.get('data'))
        trip_id = str(trip.id)
        trip_data = ReadOnlyTripSerializer(trip).data

        # Send updates to riders that are associated with
        # this trip
        await self.channel_layer.group_send(group=trip_id, message={
            'type': 'echo.message',
            'data': trip_data
        })

        # When a driver accepts a ride request, 
        # the driver needs to be added to its 
        # trip group
        if trip_id not in self.trips:
            self.trips.add(trip_id)
            await self.channel_layer.group_add(
                group = trip_id,
                channel = self.channel_name
            )

        await self.send_json({
            'type': 'MESSAGE',
            'data': trip_data
        })


    async def disconnect(self, code):
        # Remove this channel from every trip's group.
        # `channel_groups` is a list of channel operations (remove)
        channel_groups = [
            self.channel_layer.group_discard(
                group = trip,
                channel = self.channel_name
            )
            for trip in self.trips
        ]

        # Discard drivers from 'drivers' group
        user_group = await self._get_user_group(self.scope['user'])
        if user_group == 'driver':
            channel_groups.append(self.channel_layer.group_discard(
                group='drivers',
                channel=self.channel_name
            ))
        asyncio.gather(*channel_groups)

        # Remove all references to trips.
        self.trips.clear()

        await super().disconnect(code)


    @database_sync_to_async
    def _create_trip(self, content):
        serializer = TripSerializer(data=content)
        serializer.is_valid(raise_exception=True)
        trip = serializer.create(serializer.validated_data)
        return trip

    
    @database_sync_to_async
    def _update_trip(self, content):
        old_trip = Trip.objects.get(id=content.get('id'))
        serializer = TripSerializer(data=content)
        serializer.is_valid(raise_exception=True)
        new_trip = serializer.update(old_trip, serializer.validated_data)
        return new_trip


    @database_sync_to_async
    def _get_trips(self, user):
        # Returns trip_id's that are not completed 
        # for the given authenticated user
        if not user.is_authenticated:
            raise Exception('User is not authenticated')
        user_groups = user.groups.values_list('name', flat=True)
        if 'driver' in user_groups:
            return user.trips_as_driver.exclude(
                status = Trip.COMPLETED
            ).only('id').values_list('id', flat=True)
        else:
            return user.trips_as_rider.exclude(
                status = Trip.COMPLETED
            ).only('id').values_list('id', flat=True)

    @database_sync_to_async
    def _get_user_group(self, user):
        if not user.is_authenticated:
            raise Exception('User is not authenticated')
        return user.groups.first().name

        